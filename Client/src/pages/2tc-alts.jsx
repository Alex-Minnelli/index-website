  ...
  let columns = [
    {
      ...
      format: row => `${row['Tower 1']} + ${row['Tower 2']}`
    },
  ];

  function Alts(OG, ALT, mapList){
    columns = [{name: "Towers",selector: (row) => row.Number,format: row => `${row['Tower 1']} + ${row['Tower 2']}`},]
    mapList.forEach(map => {
      columns.push({
        name: map.Abbrev,
...
export function TwoTCAlts() {
  let [ALT, setALT] = useState([]);
  let [OG, setOG] = useState([]);
  let [mapList, setMapList] = useState([]);
  useEffect(() => {Axios.get('http://localhost:5000/2tc/og/abbrev').then((res) => {setOG(res.data)})}, []);
  useEffect(() => {Axios.get('http://localhost:5000/2tc/alt').then((res) => {setALT(res.data)})}, []);
